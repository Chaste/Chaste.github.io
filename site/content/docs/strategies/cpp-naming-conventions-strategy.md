...
Coding standards are crucial to enable humans to understand the code.  Both code they wrote, and code written by other developers.  This document describes naming conventions we adhere to in Chaste.  If you are writing Chaste code, please follow them!
...
required to hold the variable is allocated from the program stack, and when the variable goes out
of scope, the memory which was taken on the stack is freed. When memory is allocated dynamically
(by the programmer - using the new keyword), or if variables are declared as class attributes memory
