...
---
title : "Creating And Using A New Srn Model Tutorial"
summary: "This tutorial is automatically generated from the file cell_based/test/tutorial/TestCreatingAndUsingANewSrnModelTutorial.hpp at revision [c64c70046e25](https://github.com/Chaste/Chaste/commit/c64c70046e25e3f67b47ec3e92f29cb02d5e6830). Note that the code is given in full at the bottom of the page."
draft: false
images: []
toc: true
---
...
To implement this model we define a new SRN model, `MySrnModel`{.cpp},
which inherits from `AbstractOdeSrnModel`{.cpp} and
contains a `MyOdeSystem`{.cpp}.
...
```
The second public method overrides `CreateSrnModel()`{.cpp}. This is a
...
```
Need to re-include this after `SerializationExportWrapperForCpp.hpp`{.cpp}. This is to export the
...
```
This completes the code for `MySrnModel`{.cpp}. Note that usually this code would
...
```
Lastly, we briefly test that archiving of `MySrnModel`{.cpp} has
...
We conclude with a brief test demonstrating how `MySrnModel`{.cpp} can be used
...
Load the file `/tmp/$USER/testoutput/TestOffLatticeSimulationWithMySrnModel/results_from_time_0/results.pvd`{.cpp},
and color by `x`{.cpp}.
...
## File name `TestCreatingAndUsingANewSrnModelTutorial.hpp` 
