...
---
title : "TestCreatingAndUsingANewCellCycleModelTutorial.hpp"
description: "This tutorial is automatically generated from the file cell_based/test/tutorial/TestCreatingAndUsingANewCellCycleModelTutorial.hpp at revision [dc0fc851da33](https://github.com/Chaste/Chaste/commit/dc0fc851da33d398a30adf6f0a3033ba159c438b). Note that the code is given in full at the bottom of the page."
draft: false
images: []
toc: true
---
...
tutorials, except for `CheckReadyToDivideAndPhaseIsUpdated`{.cpp}, which defines a helper
...
G1 duration as defined in the `AbstractCellCycleModel`{.cpp} class. We will also assume that
cells divide a certain number of generations before becoming differentiated. To implement this model we define a new cell-cycle model, `MyCellCycleModel`{.cpp},
which inherits from `AbstractSimpleGenerationalCellCycleModel`{.cpp} and
overrides the `SetG1Duration()`{.cpp} method.
...
`AbstractSimpleGenerationalCellCycleModel`{.cpp}. We then archive an instance
of the `RandomNumberGenerator`{.cpp} singleton class, which is used in the
`SetG1Duration()`{.cpp} method. Note that serialization of singleton objects
...
~~~
We now set the G1 duration based on cell type. For stem and transit cells, we use the `RandomNumberGenerator`{.cpp}
...
~~~
The second public method overrides `CreateCellCycleModel()`{.cpp}. This is a
...
There are a number of things to mention regarding the `CreateCellCycleModel()`{.cpp}
...
`MyCellCycleModel`{.cpp} inherits from its base class, we will find that some of
these member variables are not set here. This is for two main reasons. First, some
of the new cell-cycle model's member variables (namely `mBirthTime`{.cpp},
`mCurrentCellCyclePhase`{.cpp}, `mReadyToDivide`{.cpp}) will already have been
correctly initialized in the new cell-cycle model's constructor. Second, the
member variable `mDimension`{.cpp} remains unset, since this cell-cycle
model does not need to know the spatial dimension, so if we were to call
`SetDimension()`{.cpp} on the new cell-cycle model an exception would be triggered;
...
may be set/overwritten as soon as `InitialiseDaughterCell()`{.cpp} is called on
...
~~~
This completes the code for `MyCellCycleModel`{.cpp}. Note that usually this code would
...
~~~
Now we construct and initialise a large number of `MyCellCycleModel`{.cpp}s and
associated cells:
~~~cpp
...
~~~
Use the helper method `CheckReadyToDivideAndPhaseIsUpdated()`{.cpp} to
...
~~~
Lastly, we briefly test that archiving of `MyCellCycleModel`{.cpp} has
been implemented correctly. Create an `OutputFileHandler`{.cpp} and use
...
~~~cpp
        {
~~~
Destroy the current instance of `SimulationTime`{.cpp} and create another instance.
...
instance of `SimulationTime`{.cpp} and creating another instance. Set
...
We conclude with a brief test demonstrating how `MyCellCycleModel`{.cpp} can be used
...
~~~cpp
        std::vector<CellPtr> cells;
~~~
We must create a shared_ptr to a `CellMutationState`{.cpp} with which to bestow the cells.
