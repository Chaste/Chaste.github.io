...
---
title : "TestSolvingOdesTutorial.hpp"
description: "This tutorial is automatically generated from the file ode/test/TestSolvingOdesTutorial.hpp at revision [dc0fc851da33](https://github.com/Chaste/Chaste/commit/dc0fc851da33d398a30adf6f0a3033ba159c438b). Note that the code is given in full at the bottom of the page."
draft: false
images: []
toc: true
---
...
our own ODE class, inheriting from `AbstractOdeSystem`{.cpp}, which implements the
`EvaluateYDerivatives()`{.cpp} method.
...
to be implemented in this concrete class. This takes in the time, a `std::vector`{.cpp} of
y values (in this case, of size 1), and a reference to a `std::vector`{.cpp} in which the
...
the method `CalculateStoppingEvent`{.cpp} from `AbstractOdeSystem`{.cpp}. For this, let us
...
the base class (`AbstractOdeSystem`{.cpp}), where it always returns false, and here we override it
...
timestep'' of 0.1. The return value is an object of type `OdeSolution`{.cpp}
...
~~~
Let's look at the results, which can be obtained from the `OdeSolution`{.cpp}
object using the methods `rGetTimes()`{.cpp} and `rGetSolutions()`{.cpp}, which
return a `std::vector`{.cpp} and a `std::vector`{.cpp} of `std::vector`{.cpp}s
respectively.
~~~cpp
...
~~~
Alternatively, we can print the solution directly to a file, using the `WriteToFile`{.cpp}
method on the {{{OdeSolution}}} class.
~~~cpp
        solutions.WriteToFile("SolvingOdesTutorial", "my_ode_solution", "sec");
~~~
Two files are written
 * `my_ode_solution.dat`{.cpp} contains the results (a header line, then one column for time and one column per variable)
 * `my_ode_solution.info`{.cpp} contains information for reading the data back, a line about the ODE solver ("`ODE SOLVER: EulerIvpOdeSolver`{.cpp}") and provenance information.
...
~~~
'''Note:''' ''when a `std::vector`{.cpp} is passed in as an initial condition
to a `Solve`{.cpp} call, it gets updated as the solve takes place''. Therefore, if
...
does not involve passing in initial conditions and returning an `OdeSolution`{.cpp}.
The `AbstractOdeSystem`{.cpp} class has a member variable called the ''state variable vector'', which can
...
Note that this ODE has a variable called `mStateVariables`{.cpp}, which has
...
## File name `TestSolvingOdesTutorial.hpp` 
