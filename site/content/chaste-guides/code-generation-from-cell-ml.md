# Using CellML electrophysiology models in Cardiac Chaste
...
# Here we add extra arguments to force chaste_codegen to use this extra argument (make Get and Set methods for
...
**Please note:** Unlike its predecessor chaste_codegen does not make use of config files, instead chaste_codegen uses [chaste_codegen command line arguments](#command-line-arguments). In order to specify conversion options, which are different from the default or project-specific options, please make use of dynamic loading (see [using dynamically loaded CellML models](#Using-dynamically-loaded-Cell-ML-models-in-Chaste-tests-or-projects)).
...
Your own code built on Chaste may also use dynamically compiled and loaded CellML models. There is now a helper class ​[CellMLLoader](class:CellMLLoader)  for this. An example is given below:
...
The following annotations are understood by chaste_codegen. See the CellML files in source:trunk/heart/src/odes/cellml for examples of their use.
...
`chaste_codegen hodgkin_huxley_squid_axon_model_1952_modified.cellml --grl1` or `chaste_codegen hodgkin_huxley_squid_axon_model_1952_modified.cellml --grl2`
...
The [cardiac executable](/chaste-guides/building-executable-apps) has the ability to automatically load cell models encoded as CellML files at run-time. In order to take advantage of this, you need (at present) to have built the executable from source yourself, as it uses your Chaste source tree to convert the CellML file into runnable code.
